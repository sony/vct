...
import numpy as np
import torch
from torch_utils import persistence_edm2 as persistence
from torch_utils import misc_edm2 as misc
    ...
    def __init__(self,
        ...
        for level, channels in enumerate(cblock):
            ...
            for idx in range(num_blocks):
                cin = cout
                cout = channels
                self.enc[f'{res}x{res}_block{idx}'] = Block(cin, cout, cemb, flavor='enc',
                                                            attention=(res in attn_resolutions), **enc_block_kwargs)
        ...
        for level, channels in reversed(list(enumerate(cblock))):
            ...
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = channels
                self.dec[f'{res}x{res}_block{idx}'] = Block(cin, cout, cemb, flavor='dec',
